...
export function useConfessions(roomId: string) {
  ...
  async function loadConfessions(showLoader = false) {
    ...
    const mapped = (data || []).map((c: any) => ({
      ...
      liked_by_me: c.confession_likes?.some((l: any) => l.user_id === userId) || false,
      
      confession_replies: c.confession_replies?.map((r: any) => ({
        ...r,
        is_mine: r.user_id === userId,
        like_count: r.confession_reply_likes?.length || 0,
        liked_by_me: r.confession_reply_likes?.some((l: any) => l.user_id === userId) || false
      }))
    }))
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel('room-' + roomId)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'confessions' }, () => loadConfessions())
      .on('postgres_changes', { event: '*', schema: 'public', table: 'confession_likes' }, () => loadConfessions())
      .on('postgres_changes', { event: '*', schema: 'public', table: 'confession_replies' }, () => loadConfessions())
      .on('postgres_changes', { event: '*', schema: 'public', table: 'confession_reply_likes' }, () => loadConfessions())
      .subscribe()
  ...
  async function createConfession(content: string) {
    ...
    const { data: profile } = await supabase.from('profiles').select('anonymous_username, anonymous_avatar').eq('id', user.id).single()
  ...
  async function addReaction(confessionId: string) {
    ...
    const { data: existing } = await supabase.from('confession_likes').select('id').eq('confession_id', confessionId).eq('user_id', userId).maybeSingle()
    if (existing) {
      await supabase.from('confession_likes').delete().eq('id', existing.id)
    } else {
      const { error } = await supabase.from('confession_likes').insert({ confession_id: confessionId, user_id: userId })
      if (error && error.code === '23505') { // 409 Conflict code in Postgres
  ...
  async function addReply(confessionId: string, content: string, parentReplyId?: string) {
    let userId = currentUserId
    if (!userId) {
       const { data } = await supabase.auth.getUser()
       userId = data.user?.id || null
  ...
  async function likeReply(replyId: string) {
    ...
    const { data: existing } = await supabase.from('confession_reply_likes').select('id').eq('reply_id', replyId).eq('user_id', userId).maybeSingle()
    if (existing) {
      await supabase.from('confession_reply_likes').delete().eq('id', existing.id)
    } else {
      const { error } = await supabase.from('confession_reply_likes').insert({ reply_id: replyId, user_id: userId })
      if (error && error.code === '23505') { // 409 Conflict code
  ...
  async function deleteReply(replyId: string) {
    ...
    setConfessions(prev => prev.map(c => ({
      ...c,
      confession_replies: c.confession_replies?.filter((r: any) => r.id !== replyId)
    })))
  ...
  async function deleteConfession(confessionId: string) {
    ...
    await supabase.from('confessions').update({ is_deleted: true }).eq('id', confessionId)
  }
